...
const slides = window.TUCA_CONTENT?.home?.map((item) => item.src) ?? [
...
function createSlides() {
  ...
  slides.forEach((slideSrc, index) => {
    ...
    image.src = slideSrc;
    image.alt = window.TUCA_CONTENT?.home?.[index]?.alt || `Imagem ${index + 1} do carrossel`;
...
[previousButton, nextButton].forEach((button) => {
  if (button) button.hidden = slides.length < 2;
});
showSlide(0);
startCarousel();
...
const sketchbookImages = window.TUCA_CONTENT?.sketchbook?.map((item) => item.src) ?? Array.from(
  { length: 23 },
  (_, index) =>
...
const sketchbookTotalPages = Math.max(1, Math.ceil(
  sketchbookImages.length / sketchbookItemsPerPage
));
...
function createSketchbookItem(
  ...
  image.src = imageSrc;
  image.alt = window.TUCA_CONTENT?.sketchbook?.[imageNumber - 1]?.alt || `Sketchbook ${imageNumber}`;
  link.setAttribute("aria-label", `Ampliar ${image.alt}`);
  image.loading = "lazy";
